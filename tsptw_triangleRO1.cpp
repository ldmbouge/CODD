...
double mst(const Matrix<int,2>& d,GNSet C,GNSet V,int src,int sink,int hops)
{
    ...
    for(auto a : t)
        for(auto b : t)
            if (a!=b) pq.insert(GE {a,b,static_cast<double>(d[a][b])});
    pq.buildHeap();
...
int main(int argc,char* argv[]) {
   ...
   const auto local = [dIn, dOut, d,C,depot](const TSPTW& s,LocalContext) -> double {
      const auto& [U,e,t,hops] = s;
      if(false) {
          if (e == depot) return 0;
          int sumIn = sum(U, [dIn](int j)
          { return dIn[j]; }) + dIn[depot];
          int sumOut = sum(U, [dOut](int j)
          { return dOut[j]; }) + dOut[e];
          return std::max(sumIn, sumOut);
      }
      else {
          return mst(d, C, C - U, depot, e, hops);
      }
