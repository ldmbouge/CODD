...
int main(int argc,char* argv[])
{
    ...
    LayerEngine<TSPTWModel> * layerEngine = gpu ? new LayerEngine<TSPTWModel> : nullptr;
    ...
    while (not currentLayer->empty())
    {
        ...
        if (gpu)
        {
           layerEngine->offloadComputation(*currentLayer,model,primalBound, DDExact, DDCtx);
           layerEngine->retrieveNodes(tmpLayer, nodeInfoNext);
        }
        else
        {
            // Labels
            ...
            std::sort(nodeInfoNext.begin(), nodeInfoNext.end(), cmpByRepBound);
        }
