...
The function $\tau$ is, unsurprisingly, another closure (which captures =sz= and =C= denoting, respectively, the number of cities and the set of cities.)
#+begin_src c++
...
The cost of a transition is also modeled with a closure that captures the edge set =es= and reads:
#+begin_src c++
  const auto scf = [&es](const TSP& s,int label) { // partial cost function 
     return es.at(GE {s.last,label});
  };
#+end_src
It simply looks up in the edge /map/ =es= the weight associated to the edge originating at vertex =s.last= and ending at vertex =label=.
