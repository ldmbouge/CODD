...
int main(int argc,char* argv[]) {
   ...
   const auto projectDom = [](const TSPTW& s) {
      return std::make_tuple(s.pos,s.hops);
   };
   ...
   if(solver == "XF") {
      BAndB engine(DD<TSPTW,
                      ...
                      decltype(scf),
                      decltype(smf),
                      decltype(eqs)>::makeDD(init,target,lgf,stf,scf,smf,eqs,C,local,projectDom,sDom),
                   w);
      engine.search(bnds);
   } else if(solver == "RF") {
      ...
      BAndBRestrictedOnly engine(DD<TSPTW,
                                    Minimize<double>,
                                    std::tuple<TSPTW::Set,int>,
                                    decltype(target),
                                    decltype(lgf),
