...
public:
    ...
    class State
    {
        MarksSet m; // set of marks
        MarksSet d; // set of distances
        gfl::i32 k;   // number of marks made
        gfl::i32 e;   // last mark
        gfl::i32 sm;  // smallest unused distance
    ...
    {
        return State(MarksSet(),MarksSet(),n,0,L+1);  // smallest unused distance should be set explicitly to L+1
    }
    ...
    State smf(State const & s1, State const & s2) noexcept
    {
        using namespace gfl;
        assert(s1.k == s2.k);
        return State(s1.m & s2.m, s1.d & s2.d, s1.k, min<i32>(s1.e,s2.e), min<i32>(s1.sm,s2.sm));
