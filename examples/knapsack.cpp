...
{
   if (argc < 3) {
      std::cout << "Usage knapsack <fname> <width> <strategyCode>\n";
      exit(1);
   }
   const char* fName = argv[1];
   const int width = argc>=3 ? atoi(argv[2]) : 64;
   const char* stratCode = argc>=4 ? argv[3] : "XFNN";

   char strat[4] = {'X','F','N','N'}; //default: relaxed first with no local or dominance
   int i = 0;
   if(stratCode[i]=='R' || stratCode[i]=='X') strat[0] = stratCode[i++];
   if(stratCode[i]=='O' || stratCode[i]=='F') strat[1] = stratCode[i++];
   if(stratCode[i]=='L' || stratCode[i]=='N') strat[2] = stratCode[i++];
   if(stratCode[i]=='D' || stratCode[i]=='N') strat[3] = stratCode[i++];
   std::cout << "strat=" << strat[0] << strat[1] << " local?=" << (strat[2]=='L') << " dom?=" << (strat[3]=='D') << std::endl;
   ...
   const auto lgf = [w](const SKS& s,DDContext) { return  Range::close(0,s.c >= w[s.n]);};
   const auto stf = [I,&w](const SKS& s,const int label) -> std::optional<SKS> {
      if (s.n < I-1) {
         return SKS { s.n+1,s.c - label * w[s.n] };
      } else return SKS { I, 0};
   };
   const auto local = strat[2]=='L' ? [I,&w,&p](const SKS& s,LocalContext) -> double {
      double nn = 0;
      int    rc = s.c;
      ...
      return nn;
   } : std::function<double(const SKS&, LocalContext)>{};
   ...
   const auto sDom = strat[2]=='L' ? [](const SKS& a,const SKS& b) -> bool { 
      return  a.c >= b.c;
   } : nullptr;
