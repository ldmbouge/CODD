...
public:
   ...
   DDGenOC(const Ordered<C>& oc) : _oc(std::move(oc)),
                       _it(_oc.begin()),
                       _end(_oc.end()) {}
   ~DDGenOC() {
...
class DDGenGNset :public DDGen {
   GNSet _gns;
   GNSet::iterator _it;
   const GNSet::iterator _end;
public:
   DDGenGNset(GNSet&& gns) : _gns(std::move(gns)),
                       _it(_gns.begin()),
                       _end(_gns.end()) {}
   DDGenGNset(const GNSet& gns) : _gns(std::move(gns)),
                       _it(_gns.begin()),
                       _end(_gns.end()) {}
   ~DDGenGNset() {
